...
## Power Cycle Erase
The firmware uses the MCU's standby mode to save energy. However, this can make it impossible to reprogram the chip using the single-wire debug interface. If that happens, you will need to perform a power cycle erase ("unbrick") with your programming software. This is not necessary when using the Python tool *rvprog.py*, as it automatically detects the issue and performs a power cycle on its own.
